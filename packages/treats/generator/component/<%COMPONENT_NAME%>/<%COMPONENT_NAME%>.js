...
class <%COMPONENT_NAME_VAR%> extends <%if(COMPONENT_TYPE === "ClassComponent")%>Component<%elsif(COMPONENT_TYPE === "ClassPureComponent")%>PureComponent<%endif%> {
    render() {
...
<%else%>
const <%COMPONENT_NAME_VAR%> = (<%if(COMPONENT_PROPS && COMPONENT_PROPS.length > 0)%>{ <%COMPONENT_PROPS.join(", ")%> }<%endif%>) => (
    <div>**Your Component Code Goes Here**</div>
);
