...
2. Next, we need to implement `getInitialState` in our `src/redux/profile/action.js`, along with `getCountries`, `getCountriesLoading`, `getCountriesSuccess`, and `getCountriesError`. You will need to install [axios][axios] to help us with the request.
```
~$ npm install axios
```
...
```
As the name implies, the `getCountries` is used to get list of countries from `https://restcountries.eu/rest/v2/all`. The `getCountries(Loading|Success|Error)` are action generator for updating data fetch status [success, error, loading] in Redux.
